...
- The repo's workflows are also available as Codex skills in `.agents/skills/` — they point back
...
- 🔴 **A rule lives in exactly ONE file. A restatement is a defect.** Other files **link** to the
  ...
  work-authorization answers: `knowledge-base/12-application-answers.md` §3 · your own hard-stop
  categories: `CLAUDE.md` §5 · retired claims: `scripts/canon.py` registry.
...
| `pre-send-check.md` | **Required** — six items, each genuinely done and marked `[x]`: FULL-TEXT, MEDIA, NO-ANSWERED-QUESTIONS, PERSON, COMPANY, ABSENCE-CLAIMS. |
