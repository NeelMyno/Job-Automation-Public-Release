...
(never submit) application forms: all of it checked by scripts, not by the agent remembering to be
...
- **Optional, for form-filling only:** `chrome-devtools-mcp`: copy `.mcp.json.example` to
  ...
  no account needed. You don't need this to draft résumés/cover letters/outreach; you only need it
  to have the agent open and fill a real application form for you to review and submit.
...
the source of truth and hand-copy from there. Use whichever you find easier, just keep them in sync.
