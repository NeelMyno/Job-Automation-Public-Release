...
- `slug:` — the path segment after your `domain:` (so `example.com/<slug>`).
- `aliases:` — other path segments that resolve to the same route (optional, comma-separated).
- `source-of-truth:` — where the deployed page's real source lives in this repo.
- `scope-line:` — the artifact's own stated scope, quoted verbatim (folded into the shows haystack).
- `shows:` — generous keywords the published artifact genuinely presents (repeatable, comma-separated).
  A distinctive work-claim noun sitting next to the link must appear here, or R10 goes AMBER.
- `never-claim-here:` — terms that may NEVER appear in a block/entry that also links this route
  (repeatable, comma-separated; matched as normalized substrings). Any hit is RED.
- `check: link-only` — the route is real but carries no claim scope (no shows/never enforcement
  beyond recognizing the slug).

A `<domain>/<slug>` whose slug has no section here is RED — that forces you to keep this file
...
published routes — delete the placeholder once you have real ones.
