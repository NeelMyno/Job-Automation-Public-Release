...
| 4 | Visa/immigration-specificity sweep (F-1/OPT/H-1B/STEM-OPT/I-20/EAD) | grep | Present only as a bracketed multiple-choice menu in `knowledge-base/02-work-authorization.md`, with no status stated as fact |
...
| 7 | PDF binary inspection | `pdftotext` + `pdfinfo` on the one shipped PDF | Text matches the fixture's own HTML source exactly; metadata shows only `Producer: WeasyPrint 68.1`, with no author field and no hidden text |
...
9. **`hooks.py`'s Stop-hook selftest was silently untestable in this build environment**, not a
   personal-data leak but a real gap: this repo had never been `git init`'d (correctly, since no push
   ...
   push and not a remote), and one of the two selftest cases was also independently found to be
