...
parallelism inside steps, and it already encodes the right depth, so do NOT stack extra multi-agent
...
JSON only, $0, no keys, no LinkedIn scraping; the crawler surfaces and tracks, and you decide what to
...
4. **Fill:** `take_snapshot` (a11y tree, stable uids; re-snapshot after EVERY navigation/render,
   since uids die) → map every field to the answer bank BEFORE filling any → `fill_form` in one batch.
   ...
   **Optional free-text fields** (About-you / anything-else / additional-context / why-us): always
   ...
   option; date inputs may CONCATENATE, so clear via the native value setter + input/change events
