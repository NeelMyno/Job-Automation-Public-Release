---
description: THE outreach session pipeline — one command, derives all pending outreach + follow-ups from the shared records, researches, drafts, gates, hands over. No arguments runs everything; "<person> <dossier>" runs one person.
...
outreach is pending — from ALL sessions, because the queue is **derived from the shared records,
...
**0. Resume point (≤5 min, fixed list — wall, same as /wave).** `ops/HANDOFF.md` + `ops/STATE.md`,
...
- All copy is written INTO the dossier's `referrals.md` (`## The messages` → `### Person` blocks —
  `outreach_format.py`'s grammar). That file is the record; chat is read off it.
